...
class Path extends ArrayList<Point> {
    ...
    static class OutPt {
        ...
        private static boolean isFirstBottomPt(Path.OutPt btmPt1, Path.OutPt btmPt2) {
            ...
            final double dx1p = Math.abs(PointOperations.getDeltaX(btmPt1.getPt(), p.getPt()));
            p = btmPt1.next;
            while (p.getPt().equals(btmPt1.getPt()) && !p.equals(btmPt1)) {
                p = p.next;
            }
            final double dx1n = Math.abs(PointOperations.getDeltaX(btmPt1.getPt(), p.getPt()));

            p = btmPt2.prev;
            while (p.getPt().equals(btmPt2.getPt()) && !p.equals(btmPt2)) {
                p = p.prev;
            }
            final double dx2p = Math.abs(PointOperations.getDeltaX(btmPt2.getPt(), p.getPt()));
            p = btmPt2.next;
            while (p.getPt().equals(btmPt2.getPt()) && p.equals(btmPt2)) {
                p = p.next;
            }
            final double dx2n = Math.abs(PointOperations.getDeltaX(btmPt2.getPt(), p.getPt()));
    ...
    public Path cleanPolygon(double distance) {
        ...
        while (op.idx == 0 && op.next != op.prev) {
            if (PointOperations.arePointsClose(op.pt, op.prev.pt, distSqrd)) {
                op = excludeOp(op);
                cnt--;
            } else if (PointOperations.arePointsClose(op.prev.pt, op.next.pt, distSqrd)) {
                excludeOp(op.next);
                op = excludeOp(op);
                cnt -= 2;
            } else if (PointOperations.slopesNearCollinear(op.prev.pt, op.pt, op.next.pt, distSqrd)) {
                op = excludeOp(op);
                cnt--;
            } else {
                op.idx = 1;
            ...
            if (ip.getY() < pt.getY() != ipNext.getY() < pt.getY()) {
                if (ip.getX() >= pt.getX()) {
                    if (ipNext.getX() > pt.getX()) {
                        ...
                        final double d = (double) (ip.getX() - pt.getX()) * (ipNext.getY() - pt.getY())
                                - (double) (ipNext.getX() - pt.getX()) * (ip.getY() - pt.getY());
                        if (d == 0) {
                            return -1;
