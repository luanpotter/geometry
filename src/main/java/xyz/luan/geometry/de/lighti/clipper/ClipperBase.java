...
abstract class ClipperBase implements Clipper {
    ...
    public boolean addPath(Path pg, PolyType polyType, boolean Closed) {
        ...
        for (;;) {
            ...
            } else if (Closed && PointOperations.slopesEqual(e.prev.getCurrent(), e.getCurrent(), e.next.getCurrent())
                    && (!isPreserveCollinear() || !PointOperations.isPt2BetweenPt1AndPt3(e.prev.getCurrent(),
    ...
    private Edge processBound(Edge e, boolean LeftBoundIsForward) {
        ...
        if (e.deltaX == Edge.HORIZONTAL) {
            ...
            if (EStart.outIdx != Edge.SKIP) {
                if (EStart.deltaX == Edge.HORIZONTAL) // ie an adjoining
                                                      // horizontal skip edge
                {
