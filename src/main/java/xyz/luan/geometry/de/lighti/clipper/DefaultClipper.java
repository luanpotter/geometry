...
class DefaultClipper extends ClipperBase {
    ...
    private static void getHorzDirection(Edge HorzEdge, Direction[] Dir, double[] Left, double[] Right) {
    ...
    private static boolean getOverlap(double a1, double a2, double b1, double b2, double[] Left, double[] Right) {
        if (a1 < a2) {
            if (b1 < b2) {
    ...
    private static int isPointInPolygon(Point pt, Path.OutPt op) {
            ...
            if (poly0y < pty != poly1y < pty) {
                if (poly0x >= ptx) {
                    if (poly1x > ptx) {
                        ...
                        final double d = (double) (poly0x - ptx) * (poly1y - pty)
    ...
    // ------------------------------------------------------------------------------
    private static boolean joinHorz(Path.OutPt op1, Path.OutPt op1b, Path.OutPt op2, Path.OutPt op2b, Point Pt,
            boolean DiscardLeft) {
        ...
        if (Dir1 == Direction.LEFT_TO_RIGHT) {
            while (op1.next.getPt().getX() <= Pt.getX() && op1.next.getPt().getX() >= op1.getPt().getX()
        ...
        if (Dir2 == Direction.LEFT_TO_RIGHT) {
            while (op2.next.getPt().getX() <= Pt.getX() && op2.next.getPt().getX() >= op2.getPt().getX()
    ...
    private static boolean joinPoints(Join j, OutRec outRec1, OutRec outRec2) {
        ...
        if (isHorizontal && j.getOffPt().equals(j.outPt1.getPt()) && j.getOffPt().equals(j.outPt2.getPt())) {
            ...
            // edges
            if (!getOverlap(op1.getPt().getX(), op1b.getPt().getX(), op2.getPt().getX(), op2b.getPt().getX(), LeftV,
                    RightV)) {
                return false;
            }
            ...
            final boolean Reverse1 = op1b.getPt().getY() > op1.getPt().getY()
                    || !PointOperations.slopesEqual(op1.getPt(), op1b.getPt(), j.getOffPt());
            if (Reverse1) {
                op1b = op1.prev;
                while (op1b.getPt().equals(op1.getPt()) && op1b != op1) {
                    op1b = op1b.prev;
            ...
            final boolean Reverse2 = op2b.getPt().getY() > op2.getPt().getY()
                    || !PointOperations.slopesEqual(op2.getPt(), op2b.getPt(), j.getOffPt());
            if (Reverse2) {
                op2b = op2.prev;
                while (op2b.getPt().equals(op2.getPt()) && op2b != op2) {
                    op2b = op2b.prev;
    ...
    public static Paths minkowskiDiff(Path poly1, Path poly2) {
        ...
        c.execute(OpType.UNION, paths, PolyFillType.NON_ZERO, PolyFillType.NON_ZERO);
        return paths;
    }
    ...
    public static Paths minkowskiSum(Path pattern, Paths paths, boolean pathIsClosed) {
        ...
        c.execute(OpType.UNION, solution, PolyFillType.NON_ZERO, PolyFillType.NON_ZERO);
        return solution;
    }
    ...
    public static Paths simplifyPolygon(Path poly, PolyFillType fillType) {
        ...
        c.execute(OpType.UNION, result, fillType, fillType);
        return result;
    }
    ...
    private void addGhostJoin(Path.OutPt Op, Point OffPt) {
        final Join j = new Join();
        j.outPt1 = Op;
        j.setOffPt(new Point(OffPt));
        ghostJoins.add(j);
    }

    // ------------------------------------------------------------------------------

    private void addJoin(Path.OutPt Op1, Path.OutPt Op2, Point OffPt) {
    ...
    private void addLocalMaxPoly(Edge e1, Edge e2, Point pt) {
        addOutPt(e1, pt);
        if (e2.windDelta == 0) {
            addOutPt(e2, pt);
        }
    ...
    private Path.OutPt addLocalMinPoly(Edge e1, Edge e2, Point pt) {
        ...
        if (prevE != null && prevE.outIdx >= 0 && Edge.topX(prevE, pt.getY()) == Edge.topX(e, pt.getY())
                && Edge.slopesEqual(e, prevE) && e.windDelta != 0 && prevE.windDelta != 0) {
    ...
    private Path.OutPt addOutPt(Edge e, Point pt) {
    ...
    private boolean doHorzSegmentsOverlap(double seg1a, double seg1b, double seg2a, double seg2b) {
        if (seg1a > seg1b) {
    ...
    private void doMaxima(Edge e) {
        ...
        while (eNext != null && eNext != eMaxPair) {
            final Point tmp = new Point(e.getTop());
            intersectEdges(e, eNext, tmp);
            e.setTop(new Point(tmp));
    ...
    @Override
    public boolean execute(OpType clipType, Paths solution) {
        return execute(clipType, solution, PolyFillType.EVEN_ODD, PolyFillType.EVEN_ODD);
    }

    @Override
    public boolean execute(OpType clipType, Paths solution, PolyFillType subjFillType, PolyFillType clipFillType) {
    ...
    @Override
    public boolean execute(OpType clipType, PolyTree polytree) {
    ...
    private void fixupOutPolygon(OutRec outRec) {
        ...
        for (;;) {
            ...
            if (pp.getPt().equals(pp.next.getPt()) || pp.getPt().equals(pp.prev.getPt())
                    || PointOperations.slopesEqual(pp.prev.getPt(), pp.getPt(), pp.next.getPt()) && (!preserveCollinear
                            || !PointOperations.isPt2BetweenPt1AndPt3(pp.prev.getPt(), pp.getPt(), pp.next.getPt()))) {
                lastOK = null;
        ...
        while (currentLM != null && currentLM.y == botY) {
            ...
            if (Op1 != null && rb.isHorizontal() && ghostJoins.size() > 0 && rb.windDelta != 0) {
                for (int i = 0; i < ghostJoins.size(); i++) {
                    ...
                    if (doHorzSegmentsOverlap(j.outPt1.getPt().getX(), j.getOffPt().getX(), rb.getBot().getX(),
                            rb.getTop().getX())) {
            ...
            if (lb.outIdx >= 0 && lb.prevInAEL != null && lb.prevInAEL.getCurrent().getX() == lb.getBot().getX()
                    && lb.prevInAEL.outIdx >= 0 && Edge.slopesEqual(lb.prevInAEL, lb) && lb.windDelta != 0
            ...
            if (lb.nextInAEL != rb) {
                ...
                if (rb.outIdx >= 0 && rb.prevInAEL.outIdx >= 0 && Edge.slopesEqual(rb.prevInAEL, rb)
    ...
    private void intersectEdges(Edge e1, Edge e2, Point pt) {
        ...
        if (e1.windDelta == 0 || e2.windDelta == 0) {
            ...
            if (e1.windDelta == 0 && e2.windDelta == 0) {
                return;
            } else if (e1.polyTyp == e2.polyTyp && e1.windDelta != e2.windDelta && clipType == OpType.UNION) {
                if (e1.windDelta == 0) {
                    if (e2Contributing) {
                        addOutPt(e1, pt);
                        if (e1Contributing) {
                ...
                if (e1.windDelta == 0 && Math.abs(e2.windCnt) == 1 && (clipType != OpType.UNION || e2.windCnt2 == 0)) {
                    addOutPt(e1, pt);
                    if (e1Contributing) {
                        e1.outIdx = Edge.UNASSIGNED;
                    }
                } else if (e2.windDelta == 0 && Math.abs(e1.windCnt) == 1
                        && (clipType != OpType.UNION || e1.windCnt2 == 0)) {
                    addOutPt(e2, pt);
                    if (e2Contributing) {
                        e2.outIdx = Edge.UNASSIGNED;
                    }
        ...
        if (e1Contributing && e2Contributing) {
            if (e1Wc != 0 && e1Wc != 1 || e2Wc != 0 && e2Wc != 1
                    || e1.polyTyp != e2.polyTyp && clipType != OpType.XOR) {
    ...
    private void intersectPoint(Edge edge1, Edge edge2, Point[] ipV) {
        final Point ip = ipV[0] = new Point();
            ...
            if (outRec1 == outRec2) {
                ...
                if (usingPolyTree) {
                    for (int j = 0; j < polyOuts.size() - 1; j++) {
                        ...
                        if (oRec.getPoints() == null || oRec.firstLeft.parseFirstLeft() != outRec1
    ...
    private void processEdgesAtTopOfScanbeam(double topY) {
            ...
            if (IsMaximaEdge) {
                ...
                if (strictlySimple) {
                    ...
                    if (e.outIdx >= 0 && e.windDelta != 0 && ePrev != null && ePrev.outIdx >= 0
                            && ePrev.getCurrent().getX() == e.getCurrent().getX() && ePrev.windDelta != 0) {
                        final Point ip = new Point(e.getCurrent());
        ...
        while (e != null) {
            if (e.isIntermediate(topY)) {
                ...
                if (ePrev != null && ePrev.getCurrent().getX() == e.getBot().getX()
                        && ePrev.getCurrent().getY() == e.getBot().getY() && op != null && ePrev.outIdx >= 0
                        && ePrev.getCurrent().getY() > ePrev.getTop().getY() && Edge.slopesEqual(e, ePrev)
                        && e.windDelta != 0 && ePrev.windDelta != 0) {
                    final Path.OutPt op2 = addOutPt(ePrev, e.getBot());
                    addJoin(op, op2, e.getTop());
                } else if (eNext != null && eNext.getCurrent().getX() == e.getBot().getX()
                        && eNext.getCurrent().getY() == e.getBot().getY() && op != null && eNext.outIdx >= 0
                        && eNext.getCurrent().getY() > eNext.getTop().getY() && Edge.slopesEqual(e, eNext)
    ...
    private void processHorizontal(Edge horzEdge, boolean isTopOfScanbeam) {
            ...
            while (e != null) {
                ...
                if (dir[0] == Direction.LEFT_TO_RIGHT && e.getCurrent().getX() <= horzRight[0]
                    ...
                    if (e == eMaxPair && IsLastHorz) {
                        if (horzEdge.outIdx >= 0) {
                            ...
                            while (eNextHorz != null) {
                                if (eNextHorz.outIdx >= 0
                                        && doHorzSegmentsOverlap(horzEdge.getBot().getX(), horzEdge.getTop().getX(),
                                                eNextHorz.getBot().getX(), eNextHorz.getTop().getX())) {
                        ...
                        final Point Pt = new Point(e.getCurrent().getX(), horzEdge.getCurrent().getY());
        ...
        if (horzEdge.nextInLML != null) {
            if (horzEdge.outIdx >= 0) {
                ...
                if (ePrev != null && ePrev.getCurrent().getX() == horzEdge.getBot().getX()
                        && ePrev.getCurrent().getY() == horzEdge.getBot().getY() && ePrev.windDelta != 0
                        && ePrev.outIdx >= 0 && ePrev.getCurrent().getY() > ePrev.getTop().getY()
                        && Edge.slopesEqual(horzEdge, ePrev)) {
                    final Path.OutPt op2 = addOutPt(ePrev, horzEdge.getBot());
                    addJoin(op1, op2, horzEdge.getTop());
                } else if (eNext != null && eNext.getCurrent().getX() == horzEdge.getBot().getX()
                        && eNext.getCurrent().getY() == horzEdge.getBot().getY() && eNext.windDelta != 0
                        && eNext.outIdx >= 0 && eNext.getCurrent().getY() > eNext.getTop().getY()
    ...
    private void updateWindingCount(Edge edge) {
        ...
        } else if (edge.windDelta == 0 && clipType != OpType.UNION) {
            edge.windCnt = 1;
